...
levelData.prototype.draw = function () {
    ...
    outputdiv.innerHTML = 'shaded tris drawn :  ' + shadedtris + ' , solid tris drawn: ' + solidtris;
    shadedtris = 0;
    solidtris = 0;
            ...
            if ((VertexList[orderedFaces[i].a].z >= .2) || (VertexList[orderedFaces[i].b].z >= .2) || (VertexList[orderedFaces[i].c].z >= .2)) {
                ...
                if (orderedFaces[i].d.angle >= 1) {
                    ...
                    */
                    ...
                    if (orderedFaces[i].d.distance > 5) {
...
function Main() {
    ...
    for (var xc = -4; xc < 8; xc = xc + 4) {
        for (var yc = -4; yc < 8; yc = yc + 4) {
    ...
    */
    ...
    for (var y = TopY; y <= BottomY; y = y + skipline) {
        ...
        if (MaxX < MinX) {
            ...
            for (var xi = MinX; xi < MaxX; xi = xi + 4) {

                context.fillStyle = 'rgb(' + Math.floor(LeftShade.r + ColorGrad.r * xi) + ',' + Math.floor(LeftShade.g + ColorGrad.g * xi) + ',' + Math.floor(LeftShade.b + ColorGrad.b * xi) + ' )';
                context.fillRect(xi, y, 4, 4);
