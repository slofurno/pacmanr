...
levelData.prototype.draw = function () {
    ...
    for (var i = 0; i < this.wallFaces.length; i++) {
        ...
        orderedFaces[i] = {
            a: this.wallFaces[i].a, b: this.wallFaces[i].b, c: this.wallFaces[i].c,
        ...
        for (var i = 0; i < this.wallFaces.length; i++) {
            ...
            if ((VertexList[this.wallFaces[i].a].z >= 1) || (VertexList[this.wallFaces[i].b].z >= 1) || (VertexList[this.wallFaces[i].c].z >= 1)) {
                if (angletocamera.angle >= 1.57) {



                    // Find the four points we are working on
                    var PointA = PointList[this.wallFaces[i].a];
                    var PointB = PointList[this.wallFaces[i].b];
                    var PointC = PointList[this.wallFaces[i].c];



                    // Render the face by looking up our vertex list
                    DrawTri(PointA.x, PointA.y, PointB.x, PointB.y, PointC.x, PointC.y, angletocamera);
 ...
 */
            ...
            for (var i = 0; i < 50; i++) {
                ...
                for (var j = 0; j < 50; j++) {
                    ...
                    if (leveldata[i][j] > 0) {
                        ...
                        //outputdiv.innerHTML += ' <Br> x:' + i + '  y: ' + j;
    ...
    function DrawTri(x1, y1, x2, y2, x3, y3, angletocamera) {
        ...
        tricolor.r = (((angletocamera.angle - 1.57) / 1.57) * tricolor.r)+(400/angletocamera.distance);


        //outputdiv.innerHTML += ' distance:  ' + angletocamera.distance + '    ' + tricolor.r + '<br>';

        // Set width and cap style
        context.fillStyle = 'rgb('+ Math.floor(Math.min(255, tricolor.r)) + ',0,0)';
